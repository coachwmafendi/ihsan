...
<div class="flex items-center gap-1" style="max-width:260px;overflow:hidden">
    <code class="min-w-0 flex-1 truncate text-[10px] leading-relaxed text-zinc-500 font-mono bg-zinc-100 rounded px-1.5 py-1 select-all"
        title="{{ $fullCode }}"
        onclick="event.preventDefault();event.stopPropagation();navigator.clipboard.writeText(this.getAttribute('title')).then(function(){var e=this.parentElement.querySelector('.copy-trigger');var s=e.querySelector('svg');var t=e.querySelector('.copied-text');s.classList.add('hidden');t.classList.remove('hidden');setTimeout(function(){s.classList.remove('hidden');t.classList.add('hidden')},1000)}.bind(this))"
    >{{ $shortCode }}</code>
    <span
        ...
        onclick="event.preventDefault();event.stopPropagation();navigator.clipboard.writeText(this.parentElement.querySelector('code').getAttribute('title')).then(function(){var s=this.querySelector('svg');var t=this.querySelector('.copied-text');s.classList.add('hidden');t.classList.remove('hidden');setTimeout(function(){s.classList.remove('hidden');t.classList.add('hidden')},1000)}.bind(this))"
    >
