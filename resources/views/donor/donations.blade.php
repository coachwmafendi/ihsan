...
@section('content')
    ...
    <div class="mb-8 grid grid-cols-2 gap-4">
        ...
        <div class="rounded-xl border border-stone-200/70 bg-white p-6">
            ...
            <p class="mt-2 text-3xl font-bold text-stone-900">{{ $donationCount }}</p>
        </div>
    </div>
    ...
    <div class="space-y-4">
            ...
            <div class="rounded-xl border border-stone-200/60 bg-white p-6 shadow-sm transition hover:shadow-md">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="mt-1 text-sm text-stone-500">{{ $donation->campaign->organization->name }}</p>
                    </div>
                    <div class="flex-shrink-0 text-right">
                        <p class="text-xl font-bold text-stone-900">RM {{ number_format($donation->gross_amount, 2) }}</p>
                        <p class="mt-1 text-xs text-stone-400">{{ $donation->created_at->format('d M Y') }}</p>
                    </div>
                </div>
        ...
        @empty
            <div class="rounded-xl border border-stone-200/60 bg-white px-8 py-16 text-center">
                ...
                <p class="text-base font-medium text-stone-700">No donations yet</p>
                <p class="mt-2 text-sm text-stone-500">Your giving history will appear here once you make a donation.</p>
            </div>
        @endforelse
    </div>
