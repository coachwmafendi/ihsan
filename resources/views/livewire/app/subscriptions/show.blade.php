...
>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_18rem] xl:grid-cols-[1fr_20rem]">
                    ...
                    @if ($subscription->donor)
                        <dl class="space-y-5">
                            ...
                            <div class="grid grid-cols-1 gap-1 sm:grid-cols-[180px_1fr] sm:gap-6">
                                ...
                                <dd class="flex items-center gap-2 text-sm font-medium text-slate-900">
                                    <span>{{ $subscription->donor->email }}</span>
                                    @if ($subscription->donor->hasValidatedEmail())
                            ...
                            <dd class="min-w-0 text-sm font-medium">
                                @if ($firstDonation?->page_url)
                                    <div class="flex min-w-0 items-start gap-2">
                                        <a href="{{ $firstDonation->page_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex min-w-0 items-start gap-1 break-all text-blue-600 hover:text-blue-700">
                                            {{ $firstDonation->page_url_display }}
                                            <x-heroicon-o-arrow-top-right-on-square class="mt-0.5 size-4 shrink-0" />
                                        </a>
                                        <x-ui.copy-button :value="$firstDonation->page_url" size="sm" class="mt-0.5 shrink-0" />
                                    </div>
                                    @if ($firstDonation->page_url_query_count > 0)
                                        <p class="mt-1 text-xs text-slate-400">
                                            · {{ $firstDonation->page_url_query_count }} {{ Str::plural('parameter', $firstDonation->page_url_query_count) }}
                                        </p>
                                    @endif
                                @else
        ...
        <div class="space-y-4">
            <x-ui.sticky-panel class="space-y-4">
