@php
    ...
    if (filled($campaign->image_path)) {
        $campaignImageRouteParameters['v'] = hash('xxh32', $campaign->image_path);
    }
    ...
    @if ($usesSecureDonationShell)
        ...
        @endif
            ...
            <div class="px-6 py-6 {{ $isPopup ? 'md:px-8 md:py-7' : '' }}">
                ...
                @if (filled($introText))
                    <div x-data="{ expanded: false }" class="mt-1 text-justify text-base/7 text-slate-600">
                        <div x-show="! expanded" class="whitespace-pre-line [&amp;>*]:inline [&amp;>p]:inline">{{ trim($introTruncated) }}{{ $isLongIntro ? '...' : '' }}</div>
                        <div x-show="expanded" x-cloak class="whitespace-pre-line">
                            {!! $introText !!}
                        </div>
                        @if ($isLongIntro)
                         ...
                         >
                            ...
                            <div class="mt-3 flex flex-col items-center gap-1.5">
                                ...
                                @elseif ($isChipDirectPost)
                                    <p class="flex items-center gap-1 text-[11px] font-medium text-slate-500">
                                        <x-heroicon-o-lock-closed class="size-3" />
                                        Secure card payment powered by CHIP
                                    </p>
                                    <div class="flex items-center gap-1.5" aria-label="Accepted cards">
                                        <x-icons.visa class="h-5 w-auto rounded" />
                                        <x-icons.mastercard class="h-5 w-auto rounded" />
                                ...
                                @else
                                    <p class="flex items-center gap-1 text-[11px] font-medium text-slate-500">
                                        <x-heroicon-o-lock-closed class="size-3" />
                                        Secure checkout — cards &amp; FPX accepted
                                    </p>
                                @endif
                            </div>
                        </div>{{-- end Step 3 --}}
