<x-layouts::landing>
    ...
    <nav class="fixed top-0 inset-x-0 z-50 bg-[#0f172a]/80 backdrop-blur-lg border-b border-white/5">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4 sm:gap-6 text-sm">
                <a href="{{ route('home') }}#features" class="hidden sm:inline text-slate-400 hover:text-white transition-colors">@lang('nav.features')</a>
                <a href="{{ route('home') }}#pricing" class="hidden sm:inline text-slate-400 hover:text-white transition-colors">@lang('nav.pricing')</a>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                @foreach (['stat1', 'stat2', 'stat3', 'stat4'] as $stat)
                    <div class="bg-slate-800/30 border border-slate-700/50 rounded-2xl p-6 text-center">
                        <div class="text-3xl sm:text-4xl font-extrabold text-teal-300">@lang("case.{$stat}.value")</div>
                        <div class="mt-2 text-xs sm:text-sm text-slate-400 leading-snug">@lang("case.{$stat}.label")</div>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <section class="pb-20 sm:pb-28">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-[1fr_300px] gap-12 lg:gap-16">
            <article class="space-y-12 min-w-0">
                {{-- About --}}
                <div>
                    <h2 class="text-xl sm:text-2xl font-bold text-white">@lang('case.about.title')</h2>
                    <p class="mt-4 text-slate-400 leading-relaxed">@lang('case.about.p1')</p>
                    <p class="mt-4 text-slate-400 leading-relaxed">@lang('case.about.p2')</p>
                </div>

                {{-- Challenge --}}
                <div>
                    <h2 class="text-xl sm:text-2xl font-bold text-white">@lang('case.challenge.title')</h2>
                    <p class="mt-4 text-slate-400 leading-relaxed">@lang('case.challenge.p1')</p>
                    <p class="mt-4 text-slate-400 leading-relaxed">@lang('case.challenge.p2')</p>

                    <blockquote class="mt-8 border-l-2 border-teal-500 pl-6">
                        <p class="text-lg sm:text-xl text-slate-200 font-medium leading-relaxed">&ldquo;@lang('case.challenge.quote')&rdquo;</p>
                        <footer class="mt-3 text-sm text-slate-500">— @lang('case.challenge.quote_by')</footer>
                    </blockquote>
                </div>

                {{-- Solution --}}
                <div>
                    <h2 class="text-xl sm:text-2xl font-bold text-white">@lang('case.solution.title')</h2>
                    <p class="mt-4 text-slate-400 leading-relaxed">@lang('case.solution.intro')</p>

                    <ul class="mt-6 space-y-3">
                        @foreach (['point1', 'point2', 'point3', 'point4', 'point5', 'point6'] as $point)
                            <li class="flex items-start gap-3 text-slate-300">
                                <svg class="w-5 h-5 text-teal-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                ...
                {{-- Results --}}
                <div>
                    <h2 class="text-xl sm:text-2xl font-bold text-white">@lang('case.results.title')</h2>
                    <p class="mt-4 text-slate-400 leading-relaxed">@lang('case.results.p1')</p>
                    <p class="mt-4 text-slate-400 leading-relaxed">@lang('case.results.p2')</p>

                    <blockquote class="mt-8 bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6 sm:p-8">
                        <svg class="w-8 h-8 text-teal-500/60" fill="currentColor" viewBox="0 0 24 24"><path d="M9.983 3v7.391c0 5.704-3.731 9.57-8.983 10.609l-.995-2.151c2.432-.917 3.995-3.638 3.995-5.849h-4v-10h9.983zm14.017 0v7.391c0 5.704-3.748 9.571-9 10.609l-.996-2.151c2.433-.917 3.996-3.638 3.996-5.849h-3.983v-10h9.983z"/></svg>
                        <p class="mt-4 text-lg sm:text-xl text-slate-200 font-medium leading-relaxed">@lang('case.results.quote')</p>
            ...
            <aside class="order-first lg:order-none">
                <div class="lg:sticky lg:top-24 bg-slate-800/30 border border-slate-700/50 rounded-2xl p-6">
                    <h2 class="text-sm font-semibold text-teal-400 tracking-widest uppercase">@lang('case.facts.title')</h2>

                    <dl class="mt-5 space-y-4 text-sm">
                        @foreach (['org', 'location', 'sector', 'founded', 'size', 'since'] as $fact)
                            <div>
                                <dt class="text-slate-500">@lang("case.facts.{$fact}_label")</dt>
                                <dd class="mt-0.5 text-slate-200 font-medium">@lang("case.facts.{$fact}")</dd>
                            </div>
                        @endforeach
                        <div>
                            <dt class="text-slate-500">@lang('case.facts.website_label')</dt>
                            <dd class="mt-0.5">
                                <a href="https://madrasahdarulfalah.org/" target="_blank" rel="noopener" class="text-teal-300 hover:text-teal-200 font-medium transition-colors">madrasahdarulfalah.org</a>
                            </dd>
                        </div>
                    </dl>

                    <a href="https://madrasahdarulfalah.org/" target="_blank" rel="noopener" class="mt-6 inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white border border-slate-700 hover:border-slate-600 rounded-full px-4 py-2 transition-colors">
    ...
    <section class="py-20 sm:py-28 text-center relative overflow-hidden">
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <h2 class="text-2xl sm:text-3xl font-bold text-white">@lang('case.cta.title')</h2>
            <p class="mt-3 text-slate-400">@lang('case.cta.subtitle')</p>
