...
class ResendLargeDonationNotifications extends Command
{
    protected $signature = 'notifications:resend-large-donation
    ...
    private function dispatchForDonations($donations): int
    {
        ...
        if ($this->option('dry-run')) {
            $this->info("Dry run — {$count} large donation notification(s) would be queued.");

            return self::SUCCESS;
        }

        if (! $this->option('force') && ! $this->confirm("Queue {$count} large donation notification(s)?")) {
            return self::SUCCESS;
        }
        ...
        $bar->finish();
        $this->newLine();
        $this->info('Large donation notifications queued.');
